				...
				<div class="table-scrollable">
					<table class="table tableitm" id="main_tb">
						...
						<tbody>
							<?php $q=1; foreach ($purchaseOrder->purchase_order_rows as $purchase_order_rows): ?>
								<tr class="tr1" row_no='<?php echo @$purchase_order_rows->id; ?>'>
									<td rowspan="2"><?= h($q) ?></td>
									<td><?php echo $this->Form->input('purchase_order_rows.'.$q.'.item_id', ['options' => $items,'label' => false,'class' => 'form-control input-sm','placeholder' => 'Item','value'=>$purchase_order_rows->item_id]); ?></td>
						...
						<tfoot>
							...
							<?php if($purchaseOrder->discount_type=='1'){ ?>
								<div class="input-group col-md-2"  id="discount_text">
								<input type="text" name="discount_per" class="form-control input-sm" placeholder="5.5"  'step'=0.01 value='<?= h($purchaseOrder->discount_per) ?>'><span class="input-group-addon">%</span>
								</div>
							<?php }else{ ?>

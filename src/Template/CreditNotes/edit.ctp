...
$(document).ready(function() { 
	...
	if(empty($ReferenceBalances) || empty($itemGroups))
	{
		?>
			...
			var purchase_acc_id=$('select[name="purchase_acc_id"] option:selected').val();
			alerT(purchase_acc_id);
			
			var url="<?php echo $this->Url->build(['controller'=>'CreditNotes','action'=>'fetchReferenceNo']); ?>";
			url=url+'/'+purchase_acc_id,
			$.ajax({
				url: url,
				type: 'GET',
				dataType: 'text'
	...
	$( document ).on( 'click', '.remove_row', function() { 
		...
		if(old_amount)
		{
			...
			var ledger_account_id=$('select[name="purchase_acc_id"] option:selected').val();
			
			
			var credit_note_id='<?php echo $credit_note_id; ?>';
			
			var url="<?php echo $this->Url->build(['controller'=>'CreditNotes','action'=>'deleteReceiptRow']); ?>";
			url=url+'/'+reference_type+'/'+old_amount+'/'+ledger_account_id+'/'+credit_note_id+'/'+reference_no,
			$.ajax({
				url: url,
				type: 'GET',
				dataType: 'text'

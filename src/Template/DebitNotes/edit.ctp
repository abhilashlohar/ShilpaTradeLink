...
$(document).ready(function() {
	...
	if(empty($ReferenceBalances) || empty($itemGroups))
	{
		?>
			...
			var received_from_id=$('select[name="sales_acc_id"] option:selected').val();
	...
	$( document ).on( 'click', '.remove_row', function() {
		...
		if(old_amount)
		{
			...
			var ledger_account_id=$('select[name="sales_acc_id"] option:selected').val();
			
			var debit_note_id='<?php echo $debit_note_id; ?>';
			
			var url="<?php echo $this->Url->build(['controller'=>'DebitNotes','action'=>'deleteReceiptRow']); ?>";
			url=url+'/'+reference_type+'/'+old_amount+'/'+ledger_account_id+'/'+debit_note_id+'/'+reference_no,
